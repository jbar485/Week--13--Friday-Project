...
15.times do |index|
  teams = []
  teams.push(Team.create!(name: Faker::TvShows::BreakingBad.episode,
    logo: Faker::Avatar.image, state: Faker::Address.state, city: Faker::Address.city, stadium: Faker::Address.state, founded: Faker::Number.number(digits: 4)))
    teams
    rand(3..15).times do |player|
      teams.each do |team|
        Player.create! :name => Faker::TvShows::RickAndMorty.character, :height =>  Faker::Number.decimal(l_digits: 2), :weight =>  Faker::Number.number(digits: 3), :position =>  Faker::Alphanumeric.alpha(number: 2), :number =>  Faker::Number.between(from: 1, to: 99), :team_id => team.id
      end
    end
  end
  p "Created #{Team.count} teams"

  #Something Is wrong with my seeding, I tried 2 different ways and neither of them are working so I also did a database dump because eveerything else is working.
